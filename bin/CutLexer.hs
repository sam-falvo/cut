...
cutPrefix               = "__CUT_"
testPrefix              = cutPrefix ++ "_"
takedownPrefix          = cutPrefix ++ "TAKEDOWN__"
bringupPrefix           = cutPrefix ++ "BRINGUP__"
setupPrefix             = cutPrefix ++ "SETUP__"

-- Because we are going to be needing them in both packed and unpacked
-- forms, we will define their packed equivalents too.

packedCUTPrefixS        = S.pack cutPrefix
...
-- Parsing tokens is done via prefix matching.  To avoid excess time
-- spent checking superfulous strings we know won't match, we "early-exit"
-- from the function if the prefix is not "__CUT_", since that's the common
-- prefix for all CUT tokens.

parseTokens :: [CutToken] -> S.ByteString -> ([CutToken], S.ByteString)
parseTokens state input =
  let isn'tCUT   = not $ packedCUTPrefixS `S.isPrefixOf` input
      isTest     = packedTestPrefixS `S.isPrefixOf` input
  ...
  in  case () of
       _ | isn'tCUT   -> (state, S.tail input)
         | isTest     -> testFor state input
