...
main = do
  ...
  let usingHeader              = CutOptions.headerSpec options

  outputHandle <- CutOptions.outputFileHandle options

  case () of
   _ | nonempty errors -> putErrors errors
     | showHelp        -> putStr usageInformation
     | showVersion     -> putStr productVersion
     | otherwise       -> do produceTestRunner files outputHandle usingHeader
                             if outputHandle == IO.stdout
                              then return ()
                              else hClose outputHandle
...
produceTestRunner :: [String] -> Handle -> String -> IO ()
produceTestRunner files toOutput usingHeader = do
  nodes <- process files

  let testRunner = CutEmit.testRunner usingHeader nodes
  case () of
   _ | nonempty nodes -> dump (S.hPut toOutput) testRunner
     | otherwise      -> return ()
